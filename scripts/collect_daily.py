...
독립성 원칙: 각 카테고리의 뉴스는 그 자체로 의미 있는 사실이다. 호르무즈 위기와의 연관이 기사에 명시되어 있지 않으면 억지로 연결하지 말 것. "호르무즈와 직접 연관인지 특정하기 어려우나" 같은 불확실한 연결 서술은 절대 금지. 해양안전 뉴스는 해양안전 뉴스로, 수산 뉴스는 수산 뉴스로 독립 보도하라.
...
{{
  ...
  "flow": {{
    "triggers": {{
      "route":     {{ "overseas": "항로·초크포인트 차단·위협 해외 상황 3~5문장. 없으면 빈 문자열.", "korea_impact": "한국 수입 항로 위협 파급 3~5문장." }},
      "source":    {{ "overseas": "공급원 교란(수출금지·제재·감산) 해외 상황 3~5문장. 없으면 빈 문자열.", "korea_impact": "한국 원자재·에너지 공급원 파급 3~5문장." }},
      "logistics": {{ "overseas": "운임·항만·선박 교란 해외 상황 3~5문장. 없으면 빈 문자열.", "korea_impact": "한국 해운·수출입 물류 파급 3~5문장." }}
    }},
    "domestic_impact": {{
      "경제·금융":  "국내 거시경제·금융시장 영향 3~5문장. 관련 기사 없으면 빈 문자열.",
      "정유·화학":  "정유·석유화학 산업 영향 3~5문장. 없으면 빈 문자열.",
      "해운·물류":  "국내 해운사·물류 업계 영향 3~5문장. 없으면 빈 문자열.",
      "식품·농업":  "식품·농업·수산 영향 3~5문장. 없으면 빈 문자열.",
      "기타산업":   "그 외 주목할 산업 영향 2~3문장. 없으면 빈 문자열."
    }}
  }},
...
서술 원칙:
- 기사 제목에서 확인할 수 있는 사실만 서술. 과장·추론·창작 금지
- 운임·유가·지수 언급 시 구체적 지표명(BDI, SCFI, Brent, WTI 등)과 대상(유조선, 컨테이너, 벌크 등)을 기사 제목에서 특정할 수 있으면 반드시 명시
- 수치 언급 시 기준 시점·비교 대상을 기사 제목에서 특정할 수 있으면 명시 (예: "전주 대비", "올해 들어", "3월 기준")
- 같은 주제의 기사들은 묶어서 일관된 흐름으로 서술. 서로 다른 주제 간에는 논리적 전환을 둘 것 (원유→곡물 등 갑작스러운 전환 금지)
...
if report_data is None:
    ...
    _LOW_QUALITY_PATTERNS = [
        ...
        # 호르무즈 억지 연결 패턴
        '호르무즈 위기와 직접 연관', '직접 연관된 조치인지',
        '직접적인 연관', '연관 여부는', '연관성은 불분명',
    ]
