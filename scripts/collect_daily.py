...
BATCH_SIZE = 20
MAX_LLM_SAMPLE_GDELT = 1000
MAX_LLM_SAMPLE_NAVER = 1000
MIN_PER_KEYWORD      = 3
...
def _build_report_prompt(gdelt_df, naver_df, date_str):
    hm_gdelt = gdelt_df[gdelt_df['relevance'].isin(['HIGH','MEDIUM'])] if len(gdelt_df) > 0 else pd.DataFrame()
    hm_naver = naver_df[naver_df['relevance'].isin(['HIGH','MEDIUM'])] if len(naver_df) > 0 else pd.DataFrame()

    # 카테고리별 기사 블록 구성
    cat_blocks = {}
    for cat in CAT_ORDER:
        titles = []
        for df in [hm_gdelt, hm_naver]:
            if len(df) == 0: continue
            sub = df[df['category'] == cat]['title'].head(5).tolist()
            titles.extend(sub)
        if titles:
            cat_blocks[cat] = titles

    cat_section = ""
    for cat, titles in cat_blocks.items():
        kr = CAT_KR.get(cat, cat)
        cat_section += f"\n[{kr}]\n"
        for t in titles:
            cat_section += f"- {t}\n"
    ...
    naver_stats_str = ""
    if len(gdelt_df) > 0:
        rc = gdelt_df['relevance'].value_counts()
        gdelt_stats_str = f"GDELT: HIGH {rc.get('HIGH',0)}, MEDIUM {rc.get('MEDIUM',0)}, LOW {rc.get('LOW',0)}, NONE {rc.get('NONE',0)}"
    if len(naver_df) > 0:
        rc = naver_df['relevance'].value_counts()
        naver_stats_str = f"네이버: HIGH {rc.get('HIGH',0)}, MEDIUM {rc.get('MEDIUM',0)}, LOW {rc.get('LOW',0)}, NONE {rc.get('NONE',0)}"

    return f"""다음은 {date_str} 해상 공급망 모니터링 기사 목록입니다.
...
아래 JSON 구조로 일별 리포트를 작성하세요:
{{
  "date": "{date_str}",
  "alert_level": "CRISIS/WARNING/CAUTION/NORMAL",
  "executive_summary": "2-3문장 핵심 요약",
  "categories": {{
    "1_Security": {{"headline": "한 줄 요약", "overseas": "해외 동향", "korea_impact": "한국 영향"}},
    "2_Safety": {{"headline": "", "overseas": "", "korea_impact": ""}},
    "3_Freight": {{"headline": "", "overseas": "", "korea_impact": ""}},
    "4_PortCargo": {{"headline": "", "overseas": "", "korea_impact": ""}},
    "5_EconFinance": {{"headline": "", "overseas": "", "korea_impact": ""}},
    "6_Seafood": {{"headline": "", "overseas": "", "korea_impact": ""}},
    "7_Shipping": {{"headline": "", "overseas": "", "korea_impact": ""}},
    "8_Logistics": {{"headline": "", "overseas": "", "korea_impact": ""}},
    "9_PortCongestion": {{"headline": "", "overseas": "", "korea_impact": ""}},
    "10_OtherIndustry": {{"headline": "", "overseas": "", "korea_impact": ""}}
  }},
  "changes": {{
    "new": [{{"issue": "신규 이슈", "detail": "상세 내용"}}],
    "escalated": [{{"issue": "심화 이슈", "detail": "상세 내용"}}],
    "resolved": [{{"issue": "해소 이슈", "detail": "상세 내용"}}]
  }},
  "outlook": "향후 1주일 전망 2-3문장"
}}"""
...
if len(gdelt_cls) > 0 or len(naver_cls) > 0:
    report_prompt = _build_report_prompt(gdelt_cls, naver_cls, str(TARGET_DATE))
    ...
    result_data = {
        ...
        "stats": {
            "gdelt_total": len(gdelt_cls),
            "naver_total": len(naver_cls),
            "gdelt_high": int(len(gdelt_cls[gdelt_cls['relevance']=='HIGH'])) if len(gdelt_cls) > 0 else 0,
            "naver_high": int(len(naver_cls[naver_cls['relevance']=='HIGH'])) if len(naver_cls) > 0 else 0,
        },
    ...
    llm = report_json
    md_lines = [
        f"# KMI 해상공급망 일일 모니터링 — {TARGET_DATE}",
        f"\n**경보 수준**: {llm.get('alert_level','N/A')}",
        f"\n## 핵심 요약\n{llm.get('executive_summary','')}",
        "\n## 카테고리별 현황"
    ]
    for cat in CAT_ORDER:
        cat_data = llm.get('categories', {}).get(cat, {})
    ...
    if any(changes.get(k) for k in ['new','escalated','resolved']):
        md_lines.append("\n## 변화 사항")
        for k, label in [('new','🆕 신규'),('escalated','⬆ 심화'),('resolved','✅ 해소')]:
            items = changes.get(k, [])
            if items:
                md_lines.append(f"\n**{label}**")
                for item in items:
                    if isinstance(item, dict):
                        md_lines.append(f"- {item.get('issue','')} — {item.get('detail','')}")
                    else:
    ...
    try:
        ...
        doc = Document()
        doc.add_heading(f'KMI 해상공급망 일일 모니터링 — {TARGET_DATE}', 0)
        doc.add_paragraph(f"경보 수준: {llm.get('alert_level','N/A')}")
        doc.add_heading('핵심 요약', 1)
        doc.add_paragraph(llm.get('executive_summary', ''))
        doc.add_heading('카테고리별 현황', 1)
        for cat in CAT_ORDER:
            cat_data = llm.get('categories', {}).get(cat, {})
            headline = cat_data.get('headline', '')
            if headline:
                kr = CAT_KR.get(cat, cat)
                doc.add_heading(kr, 2)
                doc.add_paragraph(headline)
                ov = cat_data.get('overseas', '')
                ki = cat_data.get('korea_impact', '')
                if ov: doc.add_paragraph(f"🌐 해외: {ov}")
                if ki: doc.add_paragraph(f"🇰🇷 국내: {ki}")
    ...
    print(f"✅ LLM 리포트 생성 완료 (경보: {llm.get('alert_level','N/A')})")
else:
