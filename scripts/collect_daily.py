...
def _build_report_prompt(gdelt_df, naver_df, date_str):
    # source_type 부여: GDELT=international, Naver=domestic
    gdelt_df = gdelt_df.copy(); gdelt_df['source_type'] = 'international'
    naver_df = naver_df.copy(); naver_df['source_type'] = 'domestic'
    ...
    hm       = pd.concat([hm_gdelt, hm_naver], ignore_index=True) if (len(hm_gdelt) + len(hm_naver)) > 0 else pd.DataFrame()
    hm_intl  = hm_gdelt
    hm_dom   = hm_naver

    # ── 카테고리별 기사 블록 (v3 notebook 동일) ──
    active_cats    = []
    articles_block = ""
    for cat in CAT_ORDER:
        cat_hm = hm[hm['category'] == cat] if len(hm) > 0 else pd.DataFrame()
        if len(cat_hm) == 0:
            continue
...
if len(gdelt_cls) > 0 or len(naver_cls) > 0:
    ...
    # Markdown (v3 notebook 동일 구조: flow 포함)
    ...
    for cat in CAT_ORDER:
        ...
        if ov or ki:
            kr = CAT_KR.get(cat, cat)
            md_lines.append(f"\n### {kr}")
            if ov: md_lines.append(f"🌐 해외: {ov}")
            if ki: md_lines.append(f"🇰🇷 국내: {ki}")

    flow = llm.get('flow', {})
    triggers = flow.get('triggers', {})
    if any(triggers.get(k,{}).get('overseas') or triggers.get(k,{}).get('korea_impact') for k in ['route','source','logistics']):
        md_lines.append("\n## 공급망 흐름 분석")
        label_map = {'route':'항로 위기','source':'공급원 위기','logistics':'물류 위기'}
        for k, label in label_map.items():
            t = triggers.get(k, {})
            ov = t.get('overseas','').strip()
            ki = t.get('korea_impact','').strip()
            if ov or ki:
                md_lines.append(f"\n### {label}")
                if ov: md_lines.append(f"🌐 해외: {ov}")
                if ki: md_lines.append(f"🇰🇷 국내: {ki}")
    di = flow.get('domestic_impact', {})
    if any(v.strip() for v in di.values()):
        md_lines.append("\n## 국내 산업별 영향")
        for sector, text in di.items():
            if text.strip():
                md_lines.append(f"\n**{sector}**: {text}")
    ...
    if any(changes.get(k) for k in ['new','escalated','resolved']):
        ...
        for k, label in [('new','🆕 신규'),('escalated','⬆ 심화'),('resolved','✅ 해소')]:
            ...
            if items:
                md_lines.append(f"\n**{label}**")
                for item in items:
                    md_lines.append(f"- {item}" if isinstance(item, str) else f"- {item}")

    with open(md_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(md_lines))
    print(f"  MD 저장: {md_path}")

    # DOCX (v3 notebook 동일 구조: flow 포함)
    try:
        ...
        di = flow.get('domestic_impact', {})
        if any(v.strip() for v in di.values()):
            doc.add_heading('국내 산업별 영향', 1)
            for sector, text in di.items():
                if text.strip():
                    doc.add_paragraph(f"{sector}: {text}")
        changes = llm.get('changes', {})
        if any(changes.get(k) for k in ['new','escalated','resolved']):
            doc.add_heading('변화 사항', 1)
            for k, label in [('new','신규'),('escalated','심화'),('resolved','해소')]:
                items = changes.get(k, [])
                if items:
                    doc.add_heading(label, 2)
