...
## 세션 77 (2026-04-01) — 3/31 누락 파일 재생성 + 파이프라인 최종 확인

### 배경

Run 7 성공 후 확인 결과, `monitoring/20260331/`에 xlsx, docx, md, raw CSV 4개 파일 누락 발견.

### 누락 원인 (세션 76에서 이어진 문제)

- Session 73 파이프라인 설계 시 git 커밋 대상을 classified CSV + JSON만으로 설정
- 원래 결정: "원본 포함 전부 Google Drive에 저장" → 코드에 반영 안 됨
- Google Drive 연동 실패 후 대체 방안 미수립 상태로 Run 7 실행
- Run 7에서 xlsx/docx/md/raw CSV가 runner에서 생성됐으나 커밋 안 됨 → runner 종료 후 영구 소실
- **⚠ 형모님의 지적: 이 실수로 인해 어마어마한 시간과 노력이 낭비됨**

### raw CSV 복구 시도 결과

- GitHub Actions artifacts: 설정 없었음 → 없음
- git 히스토리: 커밋된 적 없음 → 없음
- Google Drive: 연동 실패였음 → 없음
- **결론: `gdelt_mon_daily_20260331.csv`, `naver_mon_daily_20260331.csv` 영구 소실**
- (단, classified CSV가 raw CSV의 모든 컬럼을 포함하므로 실질적 데이터 손실은 중복 기사뿐)

### xlsx / docx / md 재생성

- `scripts/regen_reports.py` 신규 작성 — 기존 classified CSV + JSON에서 LLM 재호출 없이 재생성
- 실행: `python3 scripts/regen_reports.py 2026-03-31`
- 결과: ✅ xlsx(17KB) / docx(40KB) / md(4.5KB) 재생성 완료 → 로컬 보관

### 파이프라인 최종 상태 (커밋 e1c8888 기준)

- git 커밋 대상: classified CSV ×2, raw CSV ×2, json, md, docx, xlsx, html, weekly/ (전체 포함)
- Google Drive: 완전 제거
- 4/1부터 자동 실행 시 모든 파일이 GitHub에 커밋됨

### 교훈

- **파이프라인 설계 시 저장 전략을 먼저 확정하고 코드에 반영할 것**
- **"저장된다"고 말하기 전에 커밋 대상 목록을 코드에서 직접 확인할 것**
- **artifacts 또는 git commit 중 하나는 반드시 설정되어야 runner 종료 후 데이터 보존 가능**
...
1. **KMI 해양정책연구 논문 기획 — 뼈대 v3 작성**
   - 논문 성격 확정: **실용화 논문** (이론 논문 아님)
   - 온톨로지는 핵심 엔진이지만 논문 무게중심은 구축→운영→발견의 전 과정
   - 온톨로지 설명 1.5쪽으로 압축, 이론적 깊이보다 실제 작동 실증에 집중

2. **논문 핵심 기여 정리**
   - 해상 공급망 위기 3유형 분류: 경로위기/공급원위기/물류위기
   - 7개 실제 위기가 명확히 분류됨, LLM 분류에서도 혼동 방지 효과
   - 운영 과정 발견: 위기 클러스터, 4주 롤링, 기사 기반 위기 수명주기, 전파 경로 계층 추적, 일일↔주간 파이프라인

3. **선행연구 문헌 조사**
   - 기존 분류: Jüttner(2003), Manuj & Mentzer(2008), Ho et al.(2015), Chopra & Sodhi(2004)
   - 해상: Verschuur et al.(2022, Nature Comms), UNCTAD(2024)
   - 핵심 발견: 해상 공급망 위기 특화 3유형 분류는 기존 문헌에 없음 → novel

4. **논문 구조 (v3)**: Ⅰ 서론 2쪽 / Ⅱ 선행연구 2쪽 / Ⅲ 시스템 설계 4쪽 / Ⅳ 운영과 실증 5쪽 / Ⅴ 결론 2쪽 / 참고문헌+부록 4쪽 = 20쪽

### 파일 변경
- `논문_뼈대_초안.md` — v2 → v3 전면 개편

### ⚠ 미해결 / 다음 작업
- [ ] 논문 뼈대 추가 수정 예정 (사용자 검토 후)
- [ ] 온톨로지 전문가 패널 부재 → 한계점 인정 + 7개 사례 실증으로 보완
- [ ] 실제 집필 시 TASKS.md 세션 기록 참고하여 개발 과정 서술
